...
namespace FungusToast.Simulation.Analysis
{
    public class MatchupStatsAggregator
    {
        public void PrintSummary(List<GameResult> results)
        {
            ...
            var playerStats = new Dictionary<int, (string strategy, int wins, int appearances, int totalLiving, int totalDead, int mutationPointsSpent, float growthChance, float selfDeathChance, float decayMod)>();
            ...
            foreach (var result in results)
            {
                foreach (var pr in result.PlayerResults)
                {
                    ...
                    if (!playerStats.ContainsKey(playerId))
                        playerStats[playerId] = (pr.StrategyName, 0, 0, 0, 0, 0, 0f, 0f, 0f);
            ...
            Console.WriteLine("\nPer-Player Summary:");
            Console.WriteLine("Player | Strategy                             | WinRate | Avg Alive | Avg Dead | Avg MP Spent | Growth% | SelfDeath% | DecayMod");
            Console.WriteLine("-------|--------------------------------------|---------|-----------|----------|---------------|---------|------------|----------");

            foreach (var kvp in playerStats.OrderBy(k => k.Key))
            {
                int playerId = kvp.Key;
                var (strategy, wins, appearances, living, dead, mpSpent, growth, selfDeath, decayMod) = kvp.Value;
                ...
                Console.WriteLine($"{playerId,6} | {strategy,-38} | {winRate,6:F1}% | {avgLiving,9:F1} | {avgDead,8:F1} | {avgMpSpent,13:F1} | {avgGrowth,7:F2}% | {avgSelfDeath,10:F2}% | {avgDecay,8:F2}%");
            }
