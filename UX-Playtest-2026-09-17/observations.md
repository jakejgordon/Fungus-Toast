...
**Status: Fixed (2026-09-22).** Both bullets were resolved by the draft-card work recorded under 015 and verified in the same in-editor pass. The one-time-versus-persistent axis now has a `Passive` / `One-time` badge on every card. Comparison density was addressed by a different route than this observation proposed: rather than fitting cards to content or widening the comparison area, the longest descriptions were condensed, the three cards were locked to one shared effect-text size, and the blank lower region was given to the badge and the `Choose` affordance. The cards remain a fixed height.
...
**Status: In progress (2026-09-22).** First pass on the P1 layering, pending the maintainer's in-editor pass. Coachmark cards now come in two draw-order layers (`CoachmarkLayer` in `CoachmarkLayoutUtility.cs`). *HUD* cards - endgame countdown, scoreboard win condition, inspect players, pace, adaptation, camera pan, welcome - are inserted directly beneath `UI_MutationTreePanel` in `UICanvas` instead of at the end, so opening the tree covers them; the same ordering also puts them beneath every panel that follows the tree in the canvas (results screen, selection prompt, pause menu, and the draft panel once it raises itself), which addresses the "persists into the draft and results screen" part of the bullet as well. *Overlay* cards - the spend/bank-points card (it teaches a control on the open tree), the draft intro, and Auto Placement (it points at the selection prompt) - stay on top. Dragging a card keeps it in its layer (`DraggableCard` now calls `CoachmarkLayoutUtility.BringToFront`), and the shared dim backdrop is placed directly beneath the lowest visible card, fixing an off-by-one that could land it above a card it had previously sat below. Second pass (same day), covering the rest of the P1 and the P2: the card now retires itself when the countdown next advances (`UI_RightSideBar.RetireEndgameCountdownCoachmark`, called from `EndgameService.CheckForEndgameCondition`). That counts as a dismissal and marks the tip seen. The rule is that one full round with the card up is acknowledgment enough, because the X is the only explicit signal and a player who ignores it should not keep the card for the rest of the game. Its copy is shortened and ends by pointing at the sidebar countdown ("The countdown at the top of the sidebar tracks the rounds left."), so the card teaches the rule once and hands off to the compact HUD status instead of competing with it; `docs/ui/TOOLTIP_GUIDE.md` records the retire rule. A blocking modal was not used, to stay consistent with the other coachmarks. Verification so far: offline MSBuild compile of Assembly-CSharp with no new warnings. Maintainer in-editor pass (2026-09-22) found two polish issues, both fixed: the new "Game End Approaching" title was clipped to "Approachin" (coachmark titles now auto-size down to 80% before truncating, in `CoachmarkLayoutUtility.BuildCard`), and the Mutation Workshop modal lost its top border (its 1-unit outline is now the coachmark cards' 2-unit resting width, in `UI_MutationTreeToastPresenter`). A follow-up pass found cards whose fixed authored height left the last body line nearly touching the bottom edge (e.g. `Set the Pace`); `CoachmarkCard.Show` now sizes every card to its body copy with a 20-unit bottom margin (the 14-unit side inset + 6), and the welcome and camera-pan cards, which fit themselves, use the same margin.
...
**Status: In progress (2026-09-23).** First pass on both bullets, pending the maintainer's in-editor pass. P1: 016's layering alone could not fix this screenshot. The draft panel covers only the board, and the endgame card sits over the sidebar, so the card stayed on top of the draft. The draft now puts every HUD-layer coachmark on hold for as long as it is open (`CoachmarkLayoutUtility.HoldHudCoachmarks` / `ReleaseHudCoachmarks`, called from `MycovariantDraftController.ShowDraftUI` / `HideDraftUI`). A held card is taken off screen but not dismissed. It still reports `IsVisible`, so its host neither re-shows it nor marks it seen. When the draft closes, unread cards come back with their entrance, and a card the player dragged comes back where they left it. A HUD card whose host shows it during the draft waits for the release. The draft's own intro card is an Overlay card and is unaffected, so the draft is the only thing teaching while it is up. The hold spans the whole draft, including the board placement a pick like Ballistospore Discharge starts (the panel only hides itself for that), which also covers the "Auto Placement and Game End tutorials together" part of 018's tutorial-overload P1. Retirement still works during a hold: `RetireEndgameCountdownCoachmark` sees the held card as visible and closes it. Leaving the scene mid-draft ends the hold without revealing anything (`OnDestroy`), and a `SubsystemRegistration` reset clears the static state. The camera-pan and welcome hosts read the card's new `IsShowing` instead of `activeSelf`, so a held card does not look closed to them either. The rule is recorded in `docs/ui/TOOLTIP_GUIDE.md` section D. Secondary HUD panels were intentionally **not** dimmed. 011 and 014 list "board and standings stay visible" as working well, and the standings and countdown are the context a draft pick is made in. P2: when the offer holds two or more tiers of one family, and their copy is identical apart from numbers, each number that differs is set bold on a soft `State.Warning` highlight (`MycovariantTierComparison.EmphasizeTierDifferences` in Core, applied by `MycovariantDraftController.EmphasizeTierDifferences` before the shared font-size sync, because bold digits are wider). For Ballistospore Discharge I and II, only `12` and `17` light up. If the prose differs as well, nothing is marked: the player has to read both cards anyway, and a lone highlighted number would suggest that is the only difference. Cards keep their raw `Title` / `Description`, so the markup is rebuilt from scratch on every populate, reconcile, and replacement, and a card whose sibling was just drafted loses its highlight. `MycovariantTierComparisonTests` (5 tests) covers the sibling case, prose differences, untiered names, and a repository-wide guard that marking only ever wraps text and never changes it. Verification so far: the Core tests pass (121 mycovariant tests), and an offline MSBuild compile of Assembly-CSharp shows no new warnings. A related fix found during this pass (2026-09-24): a card dealt in to replace a drafted pick, or re-bound while choices were carried forward, used to show the base `Mycovariant.Description` rather than the round-aware preview that freshly dealt cards use. Corner and Aggressotropic Conduit replacements therefore listed their base tile count, without the later-draft bonus. `MycovariantCard.SetMycovariant` now takes its description from the caller, and every draft path goes through `MycovariantDraftController.GetDraftCardDescription`.
...
**Status: In progress (2026-09-19).** Partial fix in commits `a026b81` and `39dca1d`: entering any board tile-selection mode (`TileSelectionController`, `MultiTileSelectionController`, `MultiCellSelectionController`) now switches the hardware cursor to a target reticle and releases it on completion, cancel, auto-placement and teardown, so the mode is legible from the pointer itself. The magnifier lens and the cell inspector tooltip are intentionally left active during placement - the maintainer wants nearby toxins and mold inspectable while choosing a tile - so the P1 hover-inspection collision is addressed only by the cursor change, not by suppression. Groundwork for the P2 magenta treatment: the hover/selection highlight sprite was a solid-magenta placeholder that swallowed every runtime tint, and is now opaque white so the existing `State.Focus`->`Text.Primary` hover pulse renders as designed; the selectable-tile pulse colours in `UIEffectConstants` (`SelectableTilePulse*`) are still hard-coded magenta and remain open. The P1 tutorial stacking is partly addressed by 017 (2026-09-23): the draft now holds every HUD coachmark, the Game End card included, until it closes, and that covers placement a draft pick starts. So Auto Placement is the only card up during that placement. Placement started outside a draft is not covered yet. Needs the maintainer's in-editor pass.
...
**Status: In progress (2026-09-26).** All five bullets have a first pass, pending the maintainer's in-editor pass. **Tutorial P1:** 016's layering already draws HUD cards beneath the results panel, but `Inspect Board` hides the results and would have shown the card again. The results panel now holds every HUD coachmark while it is up (`CoachmarkLayoutUtility.HoldHudCoachmarks` in `UI_EndGamePanel.PreparePanelForContentBuild`) and releases them without a reveal in `OnDisable`, so no gameplay tip survives the match in either view. **Truncation P1:** the root cause was layout, not copy. On the loss screen `EnsureActionButtonsShareContainer` sized the stacked buttons at 500 wide (Exit at 330), and then `RefreshRuntimeEndGameLayout` re-applied the bottom-bar sizing to Continue, Main Menu, and Exit unconditionally. That squeezed them to 280 and cut the label off, which is why `Inspect Board` (not touched by that call) was the only full-width button in the screenshot. The redundant re-application is removed. The label is now `Choose Adaptation to Keep` (`Choose N Adaptations to Keep` for a larger capacity), so it says it opens a choice. The docked-bar width for long labels went from 248 to 300 so the new label fits there too. **Loss-explanation P2:** the loss banner puts `Campaign Lost  •  Stage N` on one line. Below it, `BuildCampaignLossSummary` states place and gap from the table: "You finished 4th of 5, 289 living cells behind AI Player 2 (326 vs 615)." It adds at most one recap line. When more of the human's cells died than are alive, and more than the winner's died, it names the decay gap. Otherwise, when the winner outspent the human by at least 20% (and 10 points or more), it names the spending gap. The generic "weren't moldy enough" line remains only as a fallback for hotseat, AI-only, or missing data. The recap states facts only; it does not prescribe a strategy. **Row-identity P2:** results rows now carry the scoreboard's YOU treatment: a Moss row tint, a Lichen left strip, and a `YOU` pill on the mold icon (`UI_GameEndPlayerResultsRow.SetPerspectivePlayer`). It is applied only when the match has exactly one human. A follow-up on 2026-09-26 also applied it to the victory-results rebuild path, which the first pass missed. All three row builders now go through `UI_EndGamePanel.CreateResultsRow`. **Progression P2:** campaign runs were already `Stage` since 002. The Moldiness card's loss line now gives the rule, "Moldiness is earned by clearing stages - none gained this run.", instead of "No moldiness gained this stage." Verification so far: an offline MSBuild compile of Assembly-CSharp shows no warnings in the touched files. The maintainer's in-editor pass on 2026-09-26 (three loss screens, 7 and 8 players) confirmed the banner copy, the one-line and two-line summaries, the YOU row, and the Moldiness line. The carry-over button was not on screen in that pass because none of those runs had Spores in Reserve.
