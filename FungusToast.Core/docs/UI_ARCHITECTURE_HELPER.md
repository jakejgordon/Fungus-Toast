...
- **Emphasize the specific blocker, and emphasize it in both places it lives**: the element under the cursor (the clicked lock / cost badge / hourglass countdown) *and*, where there is one, the thing that resolves it (the unmet prerequisite node). Leave states with nothing to point at (maxed, active surge, no-target).
