...
- **Move** (`cursor_move.png`, hotspot 16,16) - over the body of a floating card that can be dragged (section 5.11). Resolved the same way as the hand: the top raycast hit's nearest ancestor with an opinion decides, so the X button inside a draggable card still shows the hand while the rest of the card shows the move cursor. A drag in progress pushes it through `Push`/`Pop` so it holds even if the pointer outruns the card for a frame. The rule is **move = this surface goes where you drag it**; it never appears over something that only scrolls or pans.
...
- **The whole card is the drag surface.** Body text is not interactive, and a big target beats a precise title-bar grab. Nested controls keep working: Unity only starts a drag past its pointer threshold, so a click on the X still closes, and `CursorManager` gives a nested `Selectable` the hand instead of the move cursor.
