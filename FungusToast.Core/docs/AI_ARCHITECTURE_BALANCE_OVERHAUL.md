...
- **P8.1:** Cluster or compare existing behavior to find real archetype coverage,
  redundancy, counters, and gaps; do not trust authored labels alone. **Complete for
  deterministic build behavior (2026-09-10):**
  [AI_P8_ROSTER_BEHAVIOR_V1.md](second-level/AI_P8_ROSTER_BEHAVIOR_V1.md) is a
  provenance-stamped 19-strategy, 171-pair artifact generated by
  `RosterBehaviorComparison`. It finds four exact observed-build pairs. The Phase 7
  performance evidence differs within those pairs, so identical scripted spending is
  a review candidate rather than a retirement finding. No controlled matchup matrix
  yet supports counter claims; that is an explicit evidence gap for any future roster
