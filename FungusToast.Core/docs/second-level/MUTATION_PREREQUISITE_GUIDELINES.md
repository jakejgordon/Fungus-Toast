...
3. Necrohyphal Infiltration (Tier 5): Necrosporulation 1 + Detrital Enzymes 1 (a Substrate Ecology bridge — reclamation that reads any decay context). Detrital Enzymes 1 still pulls in the Aerated Frontier 10 -> Crustward/Compaction chain, so total investment stays meaningful despite the low direct levels. Effect per level was raised from 0.5% to 0.7% alongside the gate reduction. Watch acquisition timing in simulation.
4. Putrefactive Cascade (Tier 6): Necrotoxic Conversion 1 + Chemotactic Beacon 1. Necrotoxic Conversion still guarantees Putrefactive Mycotoxin 5 (and Regenerative Hyphae 1) transitively.
