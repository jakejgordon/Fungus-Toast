...
| Filament Overdrive consumes its source and creates a diagonal runner | `GameBoard.FilamentOverdriveTriggered` → `GridVisualizer.RenderBoard()` → `PlayFilamentOverdriveAnimation()` | `PlayFilamentOverdriveAnimation` | `FilamentOverdriveSourceBurnoutLeadSeconds` + `FilamentOverdriveCellRevealSeconds` per runner cell |
