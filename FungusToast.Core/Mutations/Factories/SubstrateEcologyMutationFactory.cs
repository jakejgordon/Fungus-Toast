    ...
    public static class SubstrateEcologyMutationFactory
    {
            ...
            MutationBuilderHelper helper)
        {
            ...
            helper.MakeChild(new Mutation(
                ...
                description: "Helps your colony press outward toward the crust.\n\n" +
                             $"<b>Technical:</b> Each level adds {helper.FormatPercent(GameBalance.CrustwardTropismEffectPerLevel)} growth chance to every legal orthogonal (up / down / left / right) or enabled Tendril diagonal attempt whose target is closer to the playable crust (the board edge) than its source.\n" +
            ...
            helper.MakeChild(new Mutation(
                ...
                description: "Helps your colony push through cramped territory.\n\n" +
                             $"<b>Technical:</b> Each level adds {helper.FormatPercent(GameBalance.CompactionPressureEffectPerLevel)} growth chance to every growth attempt from a living cell with {GameBalance.CompactionPressureMinimumLegalOrthogonalTargets}-{GameBalance.CompactionPressureMaximumLegalOrthogonalTargets} legal orthogonal growth targets (up / down / left / right). Fully sealed cells gain no attempt.",
            ...
            helper.MakeChildWithAnyPrerequisiteGroup(new Mutation(
                ...
                description: "Helps your colony spread through nearby dead matter.\n\n" +
                             $"<b>Technical:</b> Each level adds {helper.FormatPercent(GameBalance.DetritalEnzymesEffectPerLevel)} growth chance when the legal target has at least one orthogonally adjacent (up / down / left / right) non-toxic dead cell. Dead cells from any colony qualify, and additional dead neighbors do not stack.\n" +
            ...
            helper.MakeChild(new Mutation(
                ...
                description: "Helps your colony grow around enemy toxin fields.\n\n" +
                             $"<b>Technical:</b> Each level adds {helper.FormatPercent(GameBalance.ToxinMarginEffectPerLevel)} growth chance to every legal orthogonal (up / down / left / right) or enabled Tendril diagonal attempt whose empty target is orthogonally adjacent to at least one enemy toxin. Multiple toxins do not stack, and this bonus never grows into or removes toxin tiles.",
            ...
            helper.MakeChild(new Mutation(
                ...
                description:
                    ...
                    $"<b>Max Level Bonus:</b> Can also create Hypervariation Patches.",
            ...
            helper.MakeChild(new Mutation(
                ...
                description: "Lets a mobile toxin carry a newly grown cell into enemy territory.\n\n" +
                             $"<b>Technical:</b> Each level adds {helper.FormatPercent(GameBalance.ToxinborneSeedingEffectPerLevel)} growth chance when an empty growth target is orthogonally adjacent (up / down / left / right) to one of your toxins. After a successful qualifying growth, that toxin moves to an empty tile next to an enemy living cell, keeping only its remaining lifespan. The newly grown cell moves with it and lands in a random open orthogonal tile next to the toxin. If no such tile is open, the carried cell is lost. The carried cell cannot trigger another seeding.",
