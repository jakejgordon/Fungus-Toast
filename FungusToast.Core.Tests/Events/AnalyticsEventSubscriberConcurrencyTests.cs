...
public sealed class AnalyticsEventSubscriberConcurrencyTests
{
    /// <summary>
    /// AnalyticsEventSubscriber keeps one process-wide map of board subscriptions. Nothing in the
    /// game runs two boards at once, but a test host does: parallel xUnit collections corrupted
    /// the map and surfaced as unrelated simulation tests failing with "Operations that change
    /// non-concurrent collections must have exclusive access", landing on a different test each
    /// run. Independent boards must be able to subscribe and unsubscribe concurrently without
    /// tearing the shared map.
    /// </summary>
    [Fact]
    public void ConcurrentBoards_SubscribeAndUnsubscribeWithoutCorruptingSharedState()
    {
        ...
        Parallel.For(0, boardCount, _ =>
        {
            try
            {
                ...
                AnalyticsEventSubscriber.Unsubscribe(board, observer);
            }
