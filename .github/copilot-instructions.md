...
- **Player-facing itch.io release devlog drafting, public positioning, and outward voice (tagline, beta status, free, tone):** `FungusToast.Core/docs/ITCH_RELEASE_DEVLOG.md`
...
- Never set a text label directly on a raw `Accent.*` or `State.*` fill. Those tokens are mid-tones: they fail light text outright and clear dark text only barely, so the chip is one palette tweak from failing. Use `UIStyleTokens.Badge` (accent blended into the dark surface, accent border, `Text.Primary` label) per `UI_STYLE_GUIDE.md` section 5.12, floor chip labels at 14px, and compute the contrast ratio in **every** state of a stateful control rather than eyeballing the resting one.
