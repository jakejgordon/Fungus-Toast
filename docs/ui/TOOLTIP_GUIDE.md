...
| `MutationTreeGuidance` | mutation tree toast | Explains the persistent mutation inspector: hover to inspect, use requirement/direct-unlock chips to navigate, and Pin to hold a selection. It appears on the next mutation-tree opening after the round-1 Mutation Phase introduction is dismissed (or on the first opening when that introduction is not shown). It is then suppressed for the current game and, outside forced first-game experience, shown once per profile. |
