...
| `MutationTreeGuidance` | mutation tree toast | Explains the persistent mutation inspector: hover to inspect, use requirement/direct-unlock chips to navigate, and Pin to hold a selection. It appears on the first mutation-tree opening after the player stores mutation points. It is then suppressed for the current game and, outside forced first-game experience, shown once per profile. |
