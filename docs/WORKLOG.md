...
- **Current state:** bread photo backgrounds still use the shared visible-alpha fit + clip-budget path, but the repo now also has a repeatable validator at `scripts/validate_board_backgrounds.py` that checks serialized visible-alpha metadata against the source PNGs and sweeps current size bands for degenerate masks; the representative probe sweep and a full square sweep through `160x160` both pass on the current asset. The earlier "canonical board bounds" metadata still proved too aggressive for some shapes: large pita boards were incorrectly locked to a hard-coded inner square and then further enlarged by a render-only scale multiplier, and medium cheese boards were still effectively using the same kind of shrunken inner rectangle via oversized per-size insets; the current pita override now falls back to measured visible-alpha bounds with a light 2.5% inset and no extra scale multiplier, the cheese override now also uses measured visible-alpha bounds with a light 3% inset and no extra scale multiplier, and visible-alpha-derived backgrounds now also inscribe the requested board aspect ratio inside that candidate footprint before placement/mask sampling so square boards do not drift into extra top-bottom or left-right slack when a sprite's measured alpha bounds are slightly rectangular. Small seeded-cracker boards up to `20x20` still compose a light vertical safe-area margin on top of their sprite metadata so the top edge is less aggressive without redefining the sprite-wide footprint, and metadata-driven boards continue to bypass the new aspect-fit step. Board-edge fade, hover hit-testing, and highlight/ping overlays continue to follow the blocked-tile silhouette instead of the raw rectangular board bounds. The board renderer also now suppresses toxin overlays when no owner mold tile can be resolved, so a toxin icon should no longer render by itself if owner resolution ever fails. The remaining configuration risk is that white bread and plain cracker still rely on hard board-bounds metadata and/or non-`1.0` scale multipliers, so those bands still deserve visual verification after future art/config changes.

## Current Plan

1. Keep the visible-alpha-fit + clip-budget path as the default shape solution for bread-photo backgrounds.
2. Treat the current alpha-mask tuning baseline as `backgroundMaxTileClipFraction: 0.1`, `backgroundTileClipSampleResolution: 5`, and `backgroundScaleMultiplier: 1.0`; only keep per-background scale multipliers or hard-coded board-bounds metadata when they have been visually verified not to shrink the playable footprint.
...
4. If future boards need true rectangles/ovals with different aspect ratios in the dev start UI, split the current square-only board-size picker into independent width/height controls instead of reworking the core again.
