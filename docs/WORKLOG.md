...
- **Mycotoxin Fission (2026-08-23):** The approved Tier-5 Substrate Ecology capstone is implemented. It requires Toxin Margin 5, Mycotoxin Potentiation 5, and Putrefactive Mycotoxin 2. Each of its three levels adds +6 percentage points growth chance into an empty target orthogonally adjacent to an owned toxin; a successful qualifying growth makes the selected toxin vacate and create up to three toxins near enemy cells per level, retaining only its remaining lifespan. At max level, the colonized cell automatically grows into the vacated toxin tile without triggering another fission. Core rules, Simulation metrics, mutation tree/summary presentation, focused tests, the roster, decision log, and Unity Core DLL/PDB are updated. Manual Unity tree and visual-flow validation remains required.
- **Amplified Fission probe (2026-08-23):** The requested 2× growth bonus (+6 percentage points/level) and 3× toxin split cap (up to three toxins/level) were tested through `TST_EcologyToxinFissioner` in a 50-game 120x120, four-player, rotated-slot, nutrient/mycovariant-disabled comparison. It won 0/50 (0.0%; 187.76 average living cells, 203.54 dead cells, 65.28 toxins); `TST_Arch01_GrowthResilience` won 45/50 (90.0%; 6,272.36 living), `TST_Arch04_DriftGrowth` 5/50 (10.0%), and `TST_Arch03_FungicideSurge` 0/50. The larger toxin output is exercised but does not address the build's lack of territory; do not promote this testing strategy to the normal pool. Artifact: `toxin_fission_amplified_120x120_g50_seed230826`.
