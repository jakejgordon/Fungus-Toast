...
- **Ecology light retune / 120x120 probe (2026-08-22):** Approved pre-simulation values are Aerated Frontier +0.4 percentage points per level (20 levels) and Crustward Tropism +0.75 percentage points per level (5 levels). `TST_EcologyCrustFirst` buys Aerated Frontier 10, Crustward max, Aerated max, then follows `TST_Arch01_GrowthResilience` (Creeping Mold, Hypersystemic Regeneration, Catabolic Rebirth, Necrohyphal Infiltration); `TST_EcologyFrontierFirst` buys Aerated max, Crustward max, then that same Arch01 sequence. Both now share Arch01's ModerateEconomy fallback and Growth/Cellular Resilience priority categories. Compare them against the growth-heavy `TST_Arch01_GrowthResilience` and `TST_Arch04_DriftGrowth` on a 120x120, 50-game, slot-rotated, nutrient/mycovariant-disabled probe before repeating on 15x15.
